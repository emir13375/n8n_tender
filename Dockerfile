...
# На Railway volume монтируется поверх контейнера уже после старта
# и перетирает chown, поэтому пользователя node не используем —
# n8n запускается от root, чтобы избежать EACCES на смонтированном volume
